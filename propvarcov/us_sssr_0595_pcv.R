...
formydata <- long.dat.generation(num_subj=c(50,950), grid=40+c(0:13)*4, space = "unequal", follow.length = "short", varcov = D.1, sig = 2, prop.varcov=0.7)
...
hm2 <- gridsearch(m = hlme(fixed = as.formula(fixed.fmla), random = as.formula(rand.fmla), subject = "id", data = mydata.1, ng = 2, mixture = as.formula(mix.fmla), nwg = TRUE,verbose = FALSE), rep = 50, maxiter = 10, minit = hm1)

hm3 <- gridsearch(m = hlme(fixed = as.formula(fixed.fmla), random = as.formula(rand.fmla), subject = "id", data = mydata.1, ng = 3, mixture = as.formula(mix.fmla), nwg = TRUE,verbose = FALSE), rep = 50, maxiter = 10, minit = hm1)
...
if(bic.select==2|icl.select==2){
  result <- c(hm2$best[-1],bic.select=bic.select,icl.select=icl.select,ARI=ari,MCR=mcr,RI=rand1,JI=jaccard1,FMI=folk.mal1,mean.pprob.misc=meanpprob.misclassed,mean.pprob.correct=meanpprob.correct)
}else{
  result <- c(rep(NA,length(hm2$best[-1])),bic.select=bic.select,icl.select=icl.select,ARI=ari,MCR=mcr,RI=rand1,JI=jaccard1,FMI=folk.mal1,mean.pprob.misc=meanpprob.misclassed,mean.pprob.correct=meanpprob.correct)
...
filename = paste0("/restricted/projectnb/aging-p/yuanf/NP_Trajectories_Clustering/simulation/propvarcov/us_sssr_0595/simulation_",procid,".csv")
write.csv(t(result),file=filename)

rdname = paste0("/restricted/projectnb/aging-p/yuanf/NP_Trajectories_Clustering/simulation/propvarcov/us_sssr_0595/simulation_",procid,".RData")
