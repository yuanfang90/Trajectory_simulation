...
set.seed(20220506+procid)
D.1 <- matrix(c(0.01,0.0001,0.0001,0.0002),2,2)

formydata <- long.dat.generation(num_subj=c(200,800), grid=40+c(0:13)*4, space = "unequal", follow.length = "long", varcov = D.1, sig = 2, prop.varcov = 1.2)
...
hm1 <- hlme(fixed = as.formula(fixed.fmla), random = as.formula(rand.fmla), subject = "id", data = mydata.1,verbose = TRUE)

hm2 <- gridsearch(m = hlme(fixed = as.formula(fixed.fmla), random = as.formula(rand.fmla), subject = "id", data = mydata.1, ng = 2, mixture = as.formula(mix.fmla), nwg = TRUE,verbose = TRUE), rep = 50, maxiter = 10, minit = hm1)

hm3 <- gridsearch(m = hlme(fixed = as.formula(fixed.fmla), random = as.formula(rand.fmla), subject = "id", data = mydata.1, ng = 3, mixture = as.formula(mix.fmla), nwg = TRUE,verbose = TRUE), rep = 50, maxiter = 10, minit = hm1)
...
filename = paste0("/restricted/projectnb/aging-p/yuanf/NP_Trajectories_Clustering/simulation/balanced/case3/out_sssr/simulation_",procid,".csv")
