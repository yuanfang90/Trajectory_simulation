...
D.1 <- matrix(c(0.01,0.0001,0.0001,0.001),2,2)

formydata <- long.dat.generation(num_subj=c(200,800), grid=40+c(0:13)*4, space = "unequal", follow.length = "short", varcov = D.1, sig = 10)
...
filename = paste0("/restricted/projectnb/aging-p/yuanf/NP_Trajectories_Clustering/simulation/commonvarcov/us_mssr_2080/simulation_",procid,".csv")
write.csv(t(result),file=filename)

rdname = paste0("/restricted/projectnb/aging-p/yuanf/NP_Trajectories_Clustering/simulation/commonvarcov/us_mssr_2080/simulation_",procid,".RData")
