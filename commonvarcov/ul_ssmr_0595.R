...
formydata <- long.dat.generation(num_subj=c(50,950), grid=40+c(0:13)*4, space = "unequal", follow.length = "long", varcov = D.1, sig = 2)
...
filename = paste0("/restricted/projectnb/aging-p/yuanf/NP_Trajectories_Clustering/simulation/commonvarcov/ul_ssmr_0595/simulation_",procid,".csv")
write.csv(t(result),file=filename)

rdname = paste0("/restricted/projectnb/aging-p/yuanf/NP_Trajectories_Clustering/simulation/commonvarcov/ul_ssmr_0595/simulation_",procid,".RData")
